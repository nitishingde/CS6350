...
class RandomForestClassifier(Model, abc.ABC):
    def __init__(self, T: int = 0):
        self.hypothesises = [DecisionTreeClassifier() for _ in range(T)]

    def fit(self, df: pd.DataFrame, numerical_attributes: dict = None, T: int = 0, m: int = None, replace: bool = True, max_features: int = None, label: str = None):
        m = len(df) if m is None else m
        self.__init__(T)
        ...
        for t in range(T):
            ...
            new_df = df.sample(n=m, replace=replace).drop_duplicates(ignore_index=True)
            model = self.hypothesises[t]
            model.fit(df=new_df, numerical_attributes=numerical_attributes, max_features=max_features, heuristic='entropy')
        print('100%', end='\r')
